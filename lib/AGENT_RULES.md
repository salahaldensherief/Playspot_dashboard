...
- **Responsive Layout Rules:** Every screen MUST adapt across at least three standardized breakpoints (`mobile < 600`, `tablet 600–1024`, `desktop > 1024`) using a single shared `Breakpoints`/`ResponsiveLayout` utility. NEVER hardcode pixel-based conditionals (`if (width > 843)`) directly inside feature screens.
